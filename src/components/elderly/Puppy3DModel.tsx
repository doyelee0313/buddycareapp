  ...
  return (
    ...
    >
      ...
      <mesh position={[0, 0.2, 0.05]} rotation={[0, 0, Math.PI / 2]}>
        <capsuleGeometry args={[0.14, 0.22, 16, 16]} />
        <meshStandardMaterial color={colors.furMain} roughness={0.9} />
      </mesh>
      
      {/* Belly - smaller */}
      <mesh position={[0, 0.14, 0.08]} rotation={[0, 0, Math.PI / 2]}>
        <capsuleGeometry args={[0.09, 0.16, 16, 16]} />
...
function Scene({ mood }: Puppy3DProps) {
  return (
    <>
      ...
      {/* Mood effects */}
      {(mood === 'love' || mood === 'excited') && <FloatingHearts />}
